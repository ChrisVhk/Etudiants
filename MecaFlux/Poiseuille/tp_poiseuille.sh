...
################################################################################
#        tp_poiseuille.sh — Orchestrateur TP Hagen-Poiseuille OpenFOAM 2412
#
#  Usage:
#    ./tp_poiseuille.sh           → menu interactif
#    ./tp_poiseuille.sh all       → toutes les étapes
#    ./tp_poiseuille.sh 0         → install dépendances (venv)
#    ./tp_poiseuille.sh 1         → 01_setup.sh       (création des cas)
#    ./tp_poiseuille.sh 2         → 02_run.sh          (simulations)
#    ./tp_poiseuille.sh 3         → analyze_results.py
#    ./tp_poiseuille.sh 4         → plot_results.py
#    ./tp_poiseuille.sh 5         → compare_cases.py
#    ./tp_poiseuille.sh 6         → extract_profile.py
#    ./tp_poiseuille.sh 7         → convergence_check.py
#    ./tp_poiseuille.sh 8         → ParaView
#    ./tp_poiseuille.sh 3-7       → étapes 3 à 7
#    ./tp_poiseuille.sh 1,2,5     → étapes 1, 2 et 5
#
#  Autres scripts :
#    03_postprocess.sh            → post-traitement Python seul
#    04_clean.sh                  → nettoyage complet (cas, résultats, venv)
################################################################################
...
step_1() {
    print_header "1 — Création des cas (01_setup.sh)"
    print_info "Répertoire de travail : $SCRIPT_DIR"
    bash "$SCRIPT_DIR/01_setup.sh" || {
        print_err "Échec 01_setup.sh"
        exit 1
    }
    print_ok "Cas créés"
}
...
step_2() {
    print_header "2 — Simulations OpenFOAM (02_run.sh)"
    bash "$SCRIPT_DIR/02_run.sh" || {
        print_err "Échec 02_run.sh"
        exit 1
    }
